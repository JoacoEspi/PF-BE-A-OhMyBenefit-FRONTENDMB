...
interface UsuarioApiClient {
    ...
    suspend fun registro(
        @Body usuario: UsuarioModel
    ): Response<UsuarioApiResponse>

    //Login
    @POST("/user/login")
    suspend fun login(
        @Body usuario: UsuarioLoginModel
    ): LoginApiResponse

    //Restablecer contrasenia

    @POST("/user/forget-password")
    suspend fun restablecerContrasenia(
        @Body nuevosVal: ResetContrasenia
    ): Response<UsuarioApiResponse>

    @POST("/user/analyze-sentiment")
    suspend fun postComment(
        @Body comment: ComentarioModel
    ): Response<ComentarioModel>
    ...
    suspend fun guardarPresupuesto(
        @Body presupuesto: PresupuestoModel
    ): Response<Unit>
}